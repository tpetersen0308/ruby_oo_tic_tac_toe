module TicTacToe
  module Setupable
    ...
    private
      def self.setup_players_for_human_v_human_game(human_player, player_options)
        players = player_options.values.map do |token| 
          human_player.new(token)
        end
      end

      def self.setup_players_for_human_v_computer_game(option_selector, player_options, players, messager)
